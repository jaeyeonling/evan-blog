...
**LISTEN(서버)**
서버가 자신의 마음을 활짝 열고 연결 요청을 기다리고 있는 상태이다.

이후 클라이언트가 연결 요청을 보내기 전까지 서버는 계속 이 상태로 대기하게 된다. 즉, 적극적으로 상대방에게 대시하지 않는다는 것인데, 그래서 이 상태를 `수동 개방(Passive Open)`이라고 한다.

**SYN_SENT(클라이언트)**
클라이언트가 서버에게 연결 요청을 하면서 랜덤한 숫자인 `SYN` 값을 생성해서 보낸 상태이다. 이제 이 SYN 값을 사용하여 계속 새로운 값을 만들고 서로 확인하며 클라이언트와 서버가 `연결 상태`를 확인하게 된다.

이 경우는 클라이언트는 서버에게 적극적으로 연결해달라고 조르는 상황이므로 이 상태를 `능동 개방(Active Open)`이라고 한다.

**SYN_RECV(서버)**
서버가 클라이언트에서 보낸 SYN를 제대로 받은 상태이다. 이후 서버는 제대로 된 SYN를 받았다는 확인의 의미인 `Acknowledgement`, 즉 `ACK` 값을 만들어서 다시 클라이언트로 돌려줘야한다. 이때 `ACK`는 처음 클라이언트가 보낸 SYN에 1을 더한 값이 된다.

또한 자신만의 `SYN` 값도 다시 랜덤하게 생성하여 함께 클라이언트로 보내준다.

**ESTABLISHED(클라이언트)**
클라이언트는 자신이 처음에 보냈던 `SYN`를 알고 있기 때문에 서버가 보내준 `ACK`, 즉 `내 SYN + 1`을 확인한다. 자신이 보냈던 SYN와 이번에 받은 ACK의 차가 1이라면 제대로 연결이 되었다고 판단하는 것이다.

이후 클라이언트는 연결이 성립되었다고 판단하고 `ESTABLISHED` 상태로 들어가면서, 이번에는 서버가 보내줬던 `SYN` 값에 1을 더하여 `ACK` 값으로 다시 서버에게 보내준다.

**ESTABLISHED(서버)**
서버는 자신이 보냈던 `SYN`만 알고 있는 상황이다. 클라이언트와 마찬가지로 서버 또한 자신이 보냈던 `SYN`와 이번에 받은 ACK의 차가 1이라면 제대로 연결이 되었다고 판단하고 `ESTABLISHED` 상태로 들어가게된다.

이 과정은 `tcpdump` 유틸리티를 사용하여 디바이스의 패킷을 출력해보면 직접 확인해볼 수도 있다.
...
1. 클라이언트가 서버에게 SYN로 `1260460927`를 보냄 (SYN_SENT)
2. 서버는 자신의 SYN로 `3009967847`라는 새로운 값을 생성하고 ACK로 `1260460927 + 1`을 보냄 (SYN_RECV)
3. 클라이언트는 다시 ACK로 `3009967847 + 1`을 보냄 (클라이언트 ESTABLISHED)
4. 연결 완료 (서버 ESTABLISHED)
{% endblockquote %}
