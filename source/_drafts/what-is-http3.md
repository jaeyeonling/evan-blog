...
이런 문제들을 해결하기 위해 HTTP/3는 UDP를 기반으로 만들어진 프로토콜인 QUIC 위에서 작동하는 것을 선택한 것이다. 그렇다면 QUIC, 즉 UDP를 선택하게 되었으니 기존의 TCP가 가지고 있던 장점들까지 모두 없어진 것일까? 아무래도 이제 QUIC가 정확히 어떤 프로토콜인지를 뜯어봐야 할 것 같다.

## QUIC
`QUIC`는 TCP가 가지고 있는 이런 문제들을 해결하고 레이턴시의 한계를 뛰어넘고자 구글이 개발한 UDP 기반의 프로토콜이다.

UDP는 이름에서도 알 수 있듯이 데이터그램 방식을 사용하는 프로토콜이기 때문에 애초에 각각의 패킷 간의 순서가 존재하지 않는 독립적인 패킷을 사용한다. 또한 데이터그램 방식은 패킷의 목적지만 정해져있다면 중간 경로는 어딜 타든 신경쓰지 않기 때문에 양 종단 간의 연결 설정 또한 하지 않는다. 즉, 핸드쉐이크 과정이 필요없다는 것이다.

그렇다면 UDP를 사용하면 기존의 TCP가 가지던 신뢰성과 패킷의 무결함이 사라지는 걸까?

아니 그렇지 않다. UDP를 사용하더라도 기존의 TCP가 가지고 있던 기능을 전부 똑같이 구현할 수 있다. UDP의 진짜 장점은 바로 `커스터마이징이 용이하다`는 것이기 때문이다.
...
즉, UDP 프로토콜 자체는 TCP보다 신뢰성이 낮기도 하고 흐름 제어도 안되기 때문에 문제가 생길 수도 있지만 이후 개발자가 어플리케이션에서 구현을 어떻게 하냐에 따라서 TCP와 비슷한 수준의 기능을 가질 수도 있다는 것이다.

앞서 이야기 했듯이 애초에 TCP는 신뢰성을 확보하기위해 연결을 만들 때도 여러 번의 `RTT(Round Trip Time)`을 가져야 하고 패킷이 중간에 유실되면 해당 패킷을 다시 복구하기 위해 재전송해야 하는 등 레이턴시가 발생하게 되는데, 문제는 이 기능들이 `필수`라서 개발자가 맘대로 이런 것들을 스킵할 수도 없다. 여기서 발생하는 레이턴시들을 어떻게 더 줄여볼 시도 조차 못한다는 것이다.

TCP와 UDP의 차이를 좋은 기능이 다 들어있는 무거운 라이브러리와 필요한 기능만 들어있는 가벼운 라이브러리라고 생각하면 조금 더 이해가 빠를 것 같다. 예를 들어 JavaScript 진영에서 많이 사용하는 `lodash`와 같은 라이브러리는 기능은 무궁무진하고 사용자에게 큰 편리함을 주지만, 보통 그 안에 있는 메소드를 다 사용하지는 않을 것이다. 결국 편하긴 하지만 내가 사용하지 않는 기능까지 전부 내 JS 번들에 포함시켜야 한다는 부담이 있다.
