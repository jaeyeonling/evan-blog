...
<center>
  {% img inline '/2019/07/16/programming-math/dmorgan.png' 300 %}{% img inline '/2019/07/16/programming-math/dmorgan2.png' 300 %}
...
조건 1. 사용자가 멤버십 가입 상태가 아니고 결제 수단이 있으며, 등록된 결제 수단이 카드가 아니다
...
사실 이것도 간단한 논리식은 아니지만 그래도 조건 1, 조건 2, 조건 3만 읽어 보았을 때 이전의 조건에 비해서 어떤 상태인지 좀 더 알아보기 쉬워졌다.<small>(라고 자기합리화를 해봅니다.)</small> 필자가 이 논리식을 정리할 때 사용했던 방법이 `벤다이어그램`과 `드 모르간 법칙`이었다. 그래서 여러 개의 논리식 중 사실 `같은 명제`이지만 역의 꼴을 취하고 있는 친구들을 쉽게 찾을 수 있었고 겹치는 명제들을 골라서 합치고 좀 더 알아보기 쉬운 단위로 조건을 나누고해서 저렇게라도 만들어 놓은 것이다.
...
### 함수(Function)
`함수`도 우리가 중학교에서 배우는 개념 중 하나로 프로그래밍에서 거의 필수라고 할 수 있을 정도로 중요한 개념이다. 대부분의 프로그래머들은 우리가 매일매일 밥먹듯이 선언하는 함수가 수학에서 나온 개념이라는 것을 알고 있을 것이다.

사실 수학에서의 함수와 프로그래밍에서의 함수는 어떠한 입력을 받아서 출력을 내보내는 매커니즘은 같지만 깊게 보면 크게 다르긴하다. 수학에서의 함수는 `정의역(x)`과 `공역(y)`의 관계에 대한 추상적인 `개념`이지만 프로그래밍에서의 함수는 내부 동작까지 전부 정의해줘야 하는 `구현체`이기 때문이다.

그래도 프로그래밍에서의 함수라는 이름이 수학의 함수에서 유래한 만큼 알고 있으면 좋은 개념들도 많다! 그 중에 `전사 함수`, `단사 함수`, `전단사 함수`라는 개념이 있다.
먼저 수학의 함수에서 말하는 `정의역`은 인풋으로 들어갈 수 있는 값의 집합, `공역`은 아웃풋으로 나올 수 있는 값의 집합, `치역`은 함수의 아웃풋으로 나온 값들의 집합을 의미한다.

이렇게 얘기하면 골치아플 수 있으니 어떻게든 코드로 우겨넣어보면 이런 느낌이다.

```typescript
// 인풋의 집합이 정의역
const x = [1, 2, 3, 4, ...]; 

// Number.MIN_SAFE_INTEGER ~ Number.MAX_SAFE_INTEGER 영역이 공역
...
자 이제 `정의역`, `공역`, `치역`의 개념을 알아봤으니 `전사 함수`, `단사 함수`, `전단사 함수`를 한번 빠르게 흝어보자. 물론 수학적인 정의만 보면 머리가 아프니까 귀여운 함수 그림과 예시 코드와 함께 설명할 것이다. 먼저 `전사 함수`부터 알아보자.

<center>
  {% asset_img function-surjective.png 200 %}
</center>
...
전사 함수는 `정의역`의 원소 하나가 치역의 원소 여러 개에 대응될 수 있는 함수이다.
위의 예시 코드에서 볼 수 있듯이 $f(x) = x^2$과 같은 함수는 `1`이 인풋으로 들어오든 `-1`이 들어오든 아웃풋은 동일하게 `1`이 출력되므로 전사 함수라고 할 수 있다. 또한 전사 함수는 `공역`과 `치역`이 같은 함수이다.

<center>
  {% asset_img function-injective.png 200 %}
</center>
...
단사 함수는 `정의역`의 원소 하나가 `치역`의 원소 한개에 대응되지만 `공역`에는 `치역`의 원소 외에도 다른 원소들이 있는 함수이다.
지수함수는 인풋인 `x`가 어떤 값이 들어오던 아웃풋의 값이 겹치지 않고 $e^0 = 1$일 때를 제외하고는 모두 아웃풋이 무리수로 나온다. 결국 `치역`과 `공역`이 같다는 것을 증명할 수 없기 때문에 대표적인 단사 함수이다.

<center>
  {% asset_img function-bijective.png 200 %}
</center>
...
전단사 함수는 전사 함수의 특징인 `공역과 치역이 같다`와 단사 함수의 특징인 `정의역의 원소 하나가 치역의 원소 한개에 대응된다`라는 특징을 동시에 가지고 있는 함수이다.

즉, 정의역의 모든 원소가 치역의 모든 원소에 `일 대 일로 매칭`된다는 것이다. 제일 이해하기 쉽고 대표적인 전단사 함수는 들어간 값이 그대로 나오는 `항등함수`이며, 함수가 `전단사 함수`인 경우 우리는 $f(x) = y$일 때 $f(y) = x$를 만족하는 `역함수`를 찾을 수 있다.

자, 근데 이걸 알면 뭐가 좋길래 필자가 이렇게 설명을 한걸까?
