...
2번 방법은 어떨까? 이 방법은 상품의 ID를 배열의 인덱스로 사용하기 때문에 접근 자체는 `O(1)`만에 할 수 있다. 그러나 ID가 `100`인 요소까지 담기 위해서는 길이가 100 이상인 배열이 필요하기 때문에 메모리 공간이 낭비된다.<small>(배열은 메모리에 연속적으로 할당된다)</small>

게다가 상품의 ID는 대부분 데이터베이스에서 사용하는 `Primary Key`일 것이기 때문에 새로운 상품이 데이터베이스에 추가될수록 할당되는 ID의 값도 점점 증가할 것이고, 그러면 필요한 배열의 길이도 점점 길어질 것이라는 사실을 예상 해볼 수 있다.
...
알고리즘은 단순히 어떤 문제를 풀기 위한 방법을 외우는 것이 아니라 어떤 문제를 `효율적으로 풀기 위한 방법`을 찾는 것이다. 필자는 개인적으로, 주어진 행렬 안에서 가장 큰 정사각형을 찾는 문제를 푸는 것보다는 내가 삼중 `for`문으로 작성한 코드에서 어떻게 하면 `for`문을 하나라도 줄일 수 있을 지를 고민하는 것이 더 좋다고 생각한다.
