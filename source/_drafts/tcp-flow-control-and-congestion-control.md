...
얼핏 보면 이 방식이 굉장히 효율적이고 좋기만 한 것 같지만, Stop and Wait와 Go Back N 방식과 다르게 이 방식을 사용하는 수신 측의 버퍼에 쌓인 데이터는 연속적이지 않다는 단점이 존재한다.
...
재전송이라는 과정이 빠진 대신 재정렬이라는 과정이 추가된 것인데, 이 둘 중에 재전송이 좀 더 이득이라고 생각되면 Go Bank N 방식을, 재정렬이 좀 더 이득이라고 생각되면 Selective Repeat 방식을 사용하면된다.
