...
위 그림을 보면 수신 측이 4번 데이터부터 에러가 발생함을 감지하고 송신 측에게 `4번부터 다시 보내줘`라고 하고 있다. 이렇게 NACK를 받고나면 송신 측은 4번 데이터와 그 이후 전송했던 모든 데이터를 다시 전송하게 된다.

이때 송신 측은 비록 5번까지 전송했지만 오류가 발생하면, 오류가 발생한 4번 데이터로 되돌아가서 다시 전송해야하므로 `Go Back N`이라고 부르는 것이다.
