...
그런 이유로 많은 개발자들이 자바스크립트의 클래스를 단순한 `문법 설탕(Syntactic Sugar)`라고 이야기하지만, 사실 자바스크립트의 클래스가 단순한 문법 설탕은 아니다.
...
그 이유는 ES6에서 `class` 키워드를 통해 클래스를 지원하고 있기는 하지만, 이건 자바스크립트가 클래스 기반 언어가 되었다는게 아니기 때문이다. 결국 자바스크립트 안에서의 클래스는 클래스의 탈을 쓴 프로토타입이다.
...
### 자바스크립트가 객체를 생성하는 방법
컴퓨터 공학에서의 `객체(Object)`는 `현실의 사물을 프로그램에 반영한 것` 정도의 의미를 가진다. 객체는 여러 개의 프로퍼티와 메소드를 가지고 현실의 사물을 흉내내는 존재인 것이다.

클래스 기반 언어에서는 클래스를 생성하고 그 클래스를 사용하여 객체를 생성해야하지만, 자바스크립트는 그딴 거 없다.
...
이때 다른 언어에서는 이런 리터럴 문법을 사용하여 객체를 생성할 때 내부적으로 `클래스`를 사용하게된다. 예를 들어 파이썬 같은 경우, 딕셔너리를 리터럴로 선언하고 타입을 찍어보면 `dict` 클래스가 출력되는 것을 볼 수 있다.
...
분명히 필자는 함수만 선언했는데, 뭐 이것저것 가지고 있는 객체 녀석이 1+1으로 붙어나왔다. 함수를 생성하면 무조건 그 함수의 프로토타입 객체도 함께 생성된다는 것이 키포인트다.
...
<center>
  ...
  <small>내가 아니라 내 프로토타입 객체를 복제하렴</small>
  <br>
</center>
...
즉 `User` 함수를 사용하여 생성한 객체는 `User.prototype` 객체를 복사하여 생성된 객체이기 때문에, 이 객체들은 자신의 `__proto__` 프로퍼티로 `User.prototype` 객체를 가진다는 것이다.
...
위에서 필자는 자바스크립트에서의 객체의 의미가 두 가지라고 했었다. 첫 번째는 자료형으로써의 객체, 두 번째는 모든 객체들의 원본이 되는 객체라고 말이다. 첫 번째 의미는 객체의 생성에 대해 이야기하면서 설명했으니 이번에는 두 번째 의미에 대해서 알아볼 차례이다.

String, Boolean, Array든 뭐가 됐든 자바스크립트 내에 존재하는 모든 것들은 바로 `Object` 함수의 프로토타입인 `Object.prototype`을 시작으로 해서 복제된다.

객체들의 원본을 의미하는 Prototype Link인 `__proto__` 프로퍼티를 사용해서 쭉쭉 타고 올라가면 진짜 `Object.prototype`이 모든 객체의 조상인지 알아볼 수 있다.

일단 만만한 `String`을 사용해서 조상을 추적해보려고 하는데, `String` 객체를 생성하는 함수부터 출발할 것인지, `String` 함수를 통해 생성된 객체부터 출발할 것인지에 따라 조상까지 올라가는 길이 달라진다.

String `함수`와 String `객체`는 당연히 원본이 되는 객체도 다르기 때문이다. 필자는 그 중 String 객체를 생성할 수 있는 String 함수를 선택했다.
...
단순하게 생각 해보았을 때 원본 객체가 있고, 그 객체를 복사해서 새로운 객체를 만드는 것이 

### 프로토타입 체인을 사용한 상속
사실 ES6에 클래스가 추가되면서, 요즘에는 프로토타입 체인을 사용한 상속을 굳이 사용하는 경우는 많이 없는 것 같다. 그러나 ES6의 클래스도 결국은 프로토타입으로 구성되어있기 때문에 프로토타입을 사용한 상속이 어떤 방식으로 이루어지는지 알아야 할 필요는 있다고 본다.

방금 설명했던 프로토타입 체인에서 우리는 `원본 객체`에 대한 개념을 알아보았다. 이때 원본 객체를 복사하여 생성된 새로운 객체는 당연히 부모의 