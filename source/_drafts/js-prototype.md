...
두 번재 의미의 경우는 밑에서 다시 알아볼 예정이니, 이번 섹션에서는 자료형으로써의 객체가 생성되는 과정에 대해서 한번 보도록 하자.

자료형으로써의 객체가 가지는 의미는 일반적으로 다른 언어에서 `Map`, `ObjectArray`, `Dictionary` 등으로 불리는 `키-값(Key-Value)` 기반의 자료 구조를 말한다.
...
### 프로토타입 체인
자바스크립트 안에서 사용되는 모든 객체들은 전부 이런 프로토타입 기반 방식으로 정의되고 생성된다. 즉, `String`, `Boolean`, `Array`와 같이 우리가 일반적으로 사용하고 있는 자료형 객체들도 모두 같은 방식을 사용해서 만들었다는 것이다.
