...
실수로 `globalVariable` 변수를 `globlVariable` 변수로 오타를 냈다. 순진한 개발자는 `globalVariable` 변수의 값이 `global?`로 변경되었으리라 기대를 하겠지만 아쉽게도 그 값은 오타낸 변수명인 `globlVariable`이 가져갔다. 이런 경우도 간단한 코드에서는 디버깅이 쉽지만 조금만 코드가 복잡해져도 눈물이 흐르는 케이스이다.
