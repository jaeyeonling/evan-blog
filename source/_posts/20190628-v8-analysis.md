...
`V8`은 런타임 중에 `Profiler`라는 친구에게 함수나 변수들의 호출 빈도와 같은 데이터를 모으라고 시킨다. 이렇게 모인 데이터를 들고 `TurboFan`에게 가져가면 TurboFan은 자기 기준에 맞는 코드를 가져와서 최적화를 하는 것이다.
...
생각보다 조건이 별로 없어서 당황했다. 물론 이 메소드에서 판별하지않는 좀 더 디테일한 조건들도 존재하지만 일단 큰 틀은 이 메소드가 거의 다 가지고 있다.

`kHotAndStable`은 코드가 뜨겁고 안정적이라는 것인데, 쉽게 말하면 `자주 호출되고(뜨겁) 코드가 안 변함(안정적)`이라는 것이다.
매번 같은 행동을 수행하는 반복문 내에 있는 코드 같은 경우가 여기에 해당하기 쉽다.

`kSmallFunction`은 말 그대로 인터프리팅된 바이트 코드의 길이를 보고 특정 임계점을 넘기지 않으면 작은 함수라고 판단해서 최적화를 진행하는 것이다.
작은 함수는 크고 복잡한 함수보다 동작이 매우 추상적이거나 제한적인 확률이 높기 때문에 안정적이라고 볼 수 있다.

#### TurboFan이 일하는 모습 훔쳐보기
그럼 간단한 코드 예제를 통해 최적화가 어떤 방식으로 진행되는 지 확인해보자. 필자는 작은 함수 하나를 선언하고 반복문을 통해서 계속 호출해줄 것이다.
그러면 `ShouldOptimize` 메소드의 `ticks >= ticks_for_optimization` 조건에 걸릴 것이고, 최적화가 진행될 것이다.
...
오 최적화가 되긴 했다. `ICs with typeinfo: 3/3 (100%)`라고 적혀있는걸 보니 인라인 캐싱을 했나보다. 근데 최적화를 한 이유를 보니 `small function`이라고 적혀있다. 필자가 원했던 조건은 `OptimizationReason::kHotAndStable`으로 빠지는 것이었기 때문에 코드를 조금 바꿔서 다시 해봐야겠다.
...
그냥 아무 의미 없지만 적당히 복잡하게 만들어 보았다. `sample` 함수는 그냥 인자를 받아서 변형하고 걸러내고 순서를 뒤집어서 반환하는 역할을 수행한다. 여기서 `Turbofan`이 감시하고 있는 최적화 대상은 `sample`, `map`, `filter`, `reverse`, `Array.from` 등이다. 감시 대상이 많으므로 로그도 어마무시하게 나오기 때문에 `Turbofan`이 함수를 최적화 대상으로 `marking` 하는 부분만 가져오겠다.
...
## 마치며
최근 잘 짜여진 자바스크립트는 `C++`에 근사하는 성능을 낼 수도 있다고 한다. 자바스크립트를 사랑하는 개발자 중 한 사람으로써 자바스크립트를 잘 짜고 싶은 마음은 당연한 것 같다.
이렇게 `V8`과 같은 자바스크립트 엔진의 동작을 하나하나 뜯어보면서 내가 좋아하는 언어에 대한 이해도를 높히는 과정은 굉장히 재밌었다.<small>(학교 다닐 때 C++ 좀 많이 써볼걸...)</small>
