...
`granted`는 사용자가 이미 알림을 허용한 상태, `denied`는 거부한 상태, `default`는 아직 알림에 대한 퍼미션을 줄지말지 사용자가 결정을 하지 않은 상태이다. 따라서 우리는 퍼미션이 `default` 상태일 때 `Notification.requestPermission` 메소드를 사용하여 사용자에게 알림 노출에 대한 허가를 받아야 한다.
